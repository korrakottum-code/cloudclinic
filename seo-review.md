...
2. ✅ **แก้ Google Maps** URL ให้ตรงกับที่ตั้งจริง
3. ✅ **เพิ่ม favicon** และ apple-touch-icon ทุกหน้า
4. ✅ **Pre-render เนื้อหาสำคัญ** เป็น static HTML ครบแล้ว:
   - Standard Drip Cards (4 สูตร) ใน `index.html` และ `iv-drip-menu/`
   - Premium Drip Cards (8 สูตร) ใน `index.html` และ `iv-drip-menu/`
   - Comparison Table (12 แถว) ใน `iv-drip-menu/`
   - Reviews (8 รีวิว) ใน `reviews/`
5. ✅ **BreadcrumbList JSON-LD** ทุกหน้าย่อย (ลบ duplicate แล้ว)
6. ✅ **Google Site Verification** meta tag ใน `index.html`
7. ✅ **Clean URLs** (directory-based, ไม่มี `.html`)
8. ✅ **Fix Logo** path ใน navbar

## 🎯 สิ่งที่ควรทำต่อไป

1. **สร้าง blog articles** จาก knowledge section ที่มีอยู่แล้ว เพื่อเพิ่ม long-tail keywords
2. **เพิ่ม Review JSON-LD schema** (`AggregateRating`) ในหน้า reviews
3. **Core Web Vitals** — ตรวจสอบ LCP, CLS, FID ใน Google Search Console
